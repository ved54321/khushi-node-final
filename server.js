...
app.post('/login', function(req, res) {
    ...
    db.get("SELECT COUNT(*) as count FROM user WHERE email like '%" + email + "%' AND password='" + password + "'", function(err, rows) {
        ...
        if (err !== null) {
            res.status(700).send("User does not exist" + err);
			
        }
        if (rows.count <= 0) {
			res.status(701).send("Username and password combination is incorrect");
        ...
        if (rows.count > 0) 
		{
            res.status(702).send("Send to dashboard");
		...
		else
		{
           ...
           res.status(703).send("Send to OTP");
...
app.post('/otp', function(req, res) {
    ...
    db.get("SELECT COUNT(*) as count FROM user WHERE email like '%" + email + "%' AND otp = '" + otp + "'", function(err, rows) {
        ...
        if (err !== null) {
            ...
            res.status(704).send("User is not activated -- " + err);
        }
		if(rows.count<=0){
		
		res.status(705).send("Entered OTP is incorrect");
		}
         ...
         if (rows.count > 0) {
				...
				console.log("Inside Update");
                    if (err !== null) {
                        console.log("post err");
                        res.status(706).send("User is not activated -- " + err);;
                        console.log("feedback save");
                    }
					else{
					console.log("Update success");
					res.status(707).send("Send to Khushi");
					}
                })
            };
        });
...
server.send({
   ...
   attachment: 
   [
      {data:"<b>Greetings from KHUSHI App</b><p>Please use the below mentioned OTP for first time log in into KHUSHI App</p><p><b>OTP Number:</b> "+otp+"</p>", alternative:true}
   ]
...
app.post('/addUsers', function(req, res) {
	...
	//var totalrows=rows[0];	
		...
		if(rows.count > 0){
			res.status(701).json("Email already exist. Please contact your administrator");
		}
        else {
			...
			db.get("select id from project where project_name= '" + userproject + "'", function(err, rows) {
				...
				db.run(sqlRequest, function(err) {
					if(err !== null) {
						...
						res.status(703).json("User registered");
					}
				});
				});
					}
	...
	db.all('SELECT COUNT(*) as count FROM user where email="' + u_email + '" ORDER BY id', function(err, rows) {
		...
		if (rows.count > 0) {
			...
			db.get("select project_name from project where id IN(SELECT project_id FROM user WHERE email='" + u_email + "')", function(err, rows) {
				...
				console.log("Project_name" + pid);
				db.all('select id,question_text,project_id from question where id NOT IN(select question_id from answer where user_id =(select id from user where email="' + u_email + '")) and project_id IN(select id from project where project_name IN ("' + pid + '","All"))', function(err, row) {
					if (err !== null) {
						console.log("Inside get err");
						res.status(500).send("An error has occurred -- " + err);
					} else if (row.length == 0) {
						res.status(710).send("NO Question Configure");
...
app.post('/ans', function(req, res) {
	...
	db.get("SELECT project_id  FROM question WHERE id='" + id + "'", function(err, rows) {
		...
		//console.log(rows.count);
		if (err !== null) {
			res.status(501).send("Quesion not exist" + err);
		} else {
			...
			db.get("SELECT id,email,project_id FROM user WHERE email='" + email + "'", function(err, rows) {
				...
				db.run(sqlRequest, function(err) {
					if (err !== null) {
						...
						res.status(200).send("Your feedback is saved ");
						console.log("feedback save");
